...
const FormPaciente: React.FC<FormPacienteProps> = () => {
  const [provinces, setProvinces] = useState<
    Array<{ id: string; provincia: string }>
  >([]);
  const [distritos, setDistritos] = useState<
    Array<{ id: string; distrito: string }>
  >([]);
  ...
  const handleChangeProvincia = async (value: string) => {
    ...
    try {
      const response = await axios.get(
        `${config.backendUrl}/distritos/provincia/${value}`
      );
    ...
    try {
      if (session && session.idApoderado) {
        const response = await axios.post(
          `${config.backendUrl}/pacientes/apoderado/${session.idApoderado}/create`,
          formattedValues
        );
            ...
            >
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Ingresar codigo de certificado de nacido vivo",
                  },
                ]}
              >
                <Input style={{ marginLeft: "12px" }} />
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Ingresar nombre completo del infante",
                  },
                ]}
              >
                <Input style={{ marginLeft: "12px" }} />
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Ingresar la fecha de nacimiento",
                  },
                ]}
              >
                <DatePicker
