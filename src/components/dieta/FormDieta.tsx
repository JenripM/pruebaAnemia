...
const FormDieta = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${config.backendUrl}/predict/dieta`,
        formData
      );
  ...
  return (
    <div className="bg-white">
      ...
      <p className="px-4">
        En este formulario, podrás ingresar la frecuencia con la que consumes
        diferentes tipos de alimentos y bebidas. Esta información nos ayudará a
        entender mejor tus hábitos alimenticios y a ofrecerte recomendaciones
        más personalizadas para mejorar tu dieta. Por favor, completa todos los
        campos con la mayor precisión posible.
      </p>
          ...
          footer={[
            ...
            <Button
              ...
              onClick={async () => {
                ...
                const newConversation = await createDietaConversation(
                  dietaResultID
                );
        ...
        >
          ...
          <p className="mb-3">
            ...
            {suggestionsMutation.isSuccess && (
              <span>{suggestionsMutation.data.message}</span>
            )}
          </p>
          <List
            bordered
