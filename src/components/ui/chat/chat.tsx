...
export default function Chat() {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetcher(
        `/chatbot/conversations/${selectedChatID}/messages`
      );
      if (response.ok) {
    ...
    const sendInitialMessage = async () => {
      ...
      if (conversation.conversation.type !== ConversationType.CHAT) {
        const response = await sendMessage(selectedChatID, {
          message:
            "Quiero más recomendaciones sobre los resultados de la probabilidad de anemia de mi paciente.",
        });
        if (response) await refetchMessages();
      }
    };
  ...
  }, [messages, refetchMessages, selectedChatID]);
  ...
  return (
    ...
    >
      {contextHolder}
      <article
        className="mb-12 grid grid-rows-2 gap-y-3 overflow-hidden overflow-y-auto"
        style={{
          scrollbarWidth: "thin",
      ...
      >
        ...
        {messages && (
          <ul
            className="mt-4 flex h-fit w-full flex-col gap-2 overflow-hidden"
            id="chat-list"
          >
            {messages.map((m) => {
